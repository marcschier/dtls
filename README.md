...
> Status: under active development. Working today: the managed **DTLS 1.3** path (PSK, certificate, and raw-public-key auth) and the native **Windows Schannel DTLS 1.2** backend, both verified end to end. The OpenSSL (Linux) and Secure Transport (macOS) backends are not yet implemented. See [`docs/`](docs/) for details.
...
- DTLS **1.3** (managed; client and server) — working, with **PSK**, **X.509 certificate** (ECDSA/RSA-PSS), and **Raw Public Key** (RFC 7250) authentication.
- DTLS **1.2** on **Windows** via the native Schannel backend — working (client and server, certificate auth).
- DTLS **1.0/1.2** on Linux (OpenSSL) and macOS (Secure Transport) — planned (honest stubs today).
- Transport-agnostic datagram API with a built-in UDP `Socket` adapter and an in-memory loopback transport.
- Targets **netstandard2.1, net8.0, net9.0, net10.0**; **NativeAOT-compatible** on net10 (verified).
